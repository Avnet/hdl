...
#  Module Name:         make_uz7ev_evcc_nvme.tcl
...
if {$argc != 0} {
	# Build NVME hw platform
	set argv [list board=[lindex $argv 0] project=[lindex $argv 1] sdk=no close_project=yes dev_arch=zynqmp]
	set argc [llength $argv]
	source ./make.tcl -notrace
} else {
	# Build NVMES hw platform
   set argv [list board=uz7ev_evcc project=nvme sdk=no close_project=yes dev_arch=zynqmp]
   set argc [llength $argv]
   source ./make.tcl -notrace
}
