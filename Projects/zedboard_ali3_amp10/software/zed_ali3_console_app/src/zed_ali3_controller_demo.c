...
//                                         dependency upon
//                                         xbasic_types.h
//						Oct 15, 2015: 1.02 Updated to support Ampire 10"
...
#include "xil_cache.h"
#include "zed_accelerometer_led_plate.h"
#include "zed_dip_off.h"
#include "zed_dip_on.h"
#include "zed_draw_pen_black.h"
#include "zed_led_off.h"
#include "zed_led_on.h"
#include "zed_led_green_off.h"
#include "zed_led_green_on.h"
#include "zed_led_yellow_off.h"
#include "zed_led_yellow_on.h"
#include "zed_led_red_off.h"
#include "zed_led_red_on.h"
...
void TouchIsr(void *InstancePtr)
{
    ...
    ampire10_handle_touch_event(pDemo);
...
int zed_ali3_controller_demo_board_process(zed_ali3_controller_demo_t *pDemo)
{
	...
	int8s last_accel_x, last_accel_y, last_accel_z;
	int8s temp_diff;
	char accel_change = 0;

	/*
     * Check to see if the user is pressing PB1 (BTN8) or PB2 (BTN9) to cycle through modes.
     */
    if ((ps_gpio_get_mio_state(pDemo, 50) == 1) ||
        (ps_gpio_get_mio_state(pDemo, 51) == 1))
    {
        /*
    ...
    if (pDemo->mode == control)
    {
        ...
        // Check board input on BTNU
        if ((pDemo->button4_state == 0) && (ps_gpio_get_button(pDemo, 4) == 1))
        {
            // Old state was off, but new state is on.
            zed_ali3_controller_demo_button(pDemo, 4, 1);
            if (pDemo->bVerbose)
            {
                xil_printf("Registered ON event on BTNU control.\n\r");
            }
        }
        else if ((pDemo->button4_state == 1) && (ps_gpio_get_button(pDemo, 4) == 0))
        {
            // Old state was on, but new state is off.
            zed_ali3_controller_demo_button(pDemo, 4, 0);

            if (pDemo->bVerbose)
            {
                xil_printf("Registered OFF event on BTNU control.\n\r");
            }
        }

        // Check board input on SW0
        ...
        // Check board input on SW4
        if ((pDemo->switch4_state == 0) && (ps_gpio_get_switch(pDemo, 4) == 1))
        {
            // Old state was off, but new state is on.
            zed_ali3_controller_demo_switch(pDemo, 4, 1);
            if (pDemo->bVerbose)
            {
                xil_printf("Registered ON event on SW4 control.\n\r");
            }
        }
        else if ((pDemo->switch4_state == 1) && (ps_gpio_get_switch(pDemo, 4) == 0))
        {
            // Old state was on, but new state is off.
            zed_ali3_controller_demo_switch(pDemo, 4, 0);

            if (pDemo->bVerbose)
            {
                xil_printf("Registered OFF event on SW4 control.\n\r");
            }
        }

        // Check board input on SW5
        if ((pDemo->switch5_state == 0) && (ps_gpio_get_switch(pDemo, 5) == 1))
        {
            // Old state was off, but new state is on.
            zed_ali3_controller_demo_switch(pDemo, 5, 1);
            if (pDemo->bVerbose)
            {
                xil_printf("Registered ON event on SW5 control.\n\r");
            }
        }
        else if ((pDemo->switch5_state == 1) && (ps_gpio_get_switch(pDemo, 5) == 0))
        {
            // Old state was on, but new state is off.
            zed_ali3_controller_demo_switch(pDemo, 5, 0);

            if (pDemo->bVerbose)
            {
                xil_printf("Registered OFF event on SW5 control.\n\r");
            }
        }

        // Check board input on SW6
        if ((pDemo->switch6_state == 0) && (ps_gpio_get_switch(pDemo, 6) == 1))
        {
            // Old state was off, but new state is on.
            zed_ali3_controller_demo_switch(pDemo, 6, 1);
            if (pDemo->bVerbose)
            {
                xil_printf("Registered ON event on SW6 control.\n\r");
            }
        }
        else if ((pDemo->switch6_state == 1) && (ps_gpio_get_switch(pDemo, 6) == 0))
        {
            // Old state was on, but new state is off.
            zed_ali3_controller_demo_switch(pDemo, 6, 0);

            if (pDemo->bVerbose)
            {
                xil_printf("Registered OFF event on SW6 control.\n\r");
            }
        }

        // Check board input on SW7
        if ((pDemo->switch7_state == 0) && (ps_gpio_get_switch(pDemo, 7) == 1))
        {
            // Old state was off, but new state is on.
            zed_ali3_controller_demo_switch(pDemo, 7, 1);
            if (pDemo->bVerbose)
            {
                xil_printf("Registered ON event on SW7 control.\n\r");
            }
        }
        else if ((pDemo->switch7_state == 1) && (ps_gpio_get_switch(pDemo, 7) == 0))
        {
            // Old state was on, but new state is off.
            zed_ali3_controller_demo_switch(pDemo, 7, 0);

            if (pDemo->bVerbose)
            {
                xil_printf("Registered OFF event on SW7 control.\n\r");
            }
        }
    }

    // Poll accelerometer
    last_accel_x = (int8s)pDemo->accel_x;
    last_accel_y = (int8s)pDemo->accel_y;
    last_accel_z = (int8s)pDemo->accel_z;

    XScuGic_Disable(&(pDemo->Intc), pDemo->uInterruptId_IRQ_Touch);
    accelerometer_poll(pDemo);
    XScuGic_Enable(&(pDemo->Intc), pDemo->uInterruptId_IRQ_Touch);

    temp_diff = (int8s)((int8s)pDemo->accel_x - (int8s)last_accel_x);


    if ((temp_diff > ACCEL_SENSITIVITY) && (temp_diff < (256 - ACCEL_SENSITIVITY))){
    	accel_change = 1;
    	pDemo->accel_x_delta = (int8s)temp_diff;
    }
    else{
    	pDemo->accel_x_delta = 0;
    }

    temp_diff = pDemo->accel_y - last_accel_y;

	if ((temp_diff > ACCEL_SENSITIVITY) && (temp_diff < (256 - ACCEL_SENSITIVITY))){
		accel_change = 1;
		pDemo->accel_y_delta = (int8s)temp_diff;
	}
	else{
	    	pDemo->accel_y_delta = 0;
	    }


	temp_diff = pDemo->accel_z - last_accel_z;

	if ((temp_diff > ACCEL_SENSITIVITY) && (temp_diff < (256 - ACCEL_SENSITIVITY))){
		accel_change = 1;
		pDemo->accel_z_delta = (int8s)temp_diff;
	}
	else{
	    	pDemo->accel_z_delta = 0;
	    }
	...
	{
		if (accel_change==1) {
			xil_printf("Accelerometer data change (X,Y,Z) = %d,%d,%d\n\r",pDemo->accel_x,pDemo->accel_y,pDemo->accel_z);
		}
	}

	// Update the display with adjusted indicators.
	zed_ali3_controller_demo_accel_indication(pDemo);
...
int zed_ali3_controller_demo_button(zed_ali3_controller_demo_t *pDemo, int button_number, int button_state)
{
    ...
    {
        pDemo->button4_state = button_state;

        graphic.location_x = BUTTON4_POSITION_X;
        graphic.location_y = BUTTON4_POSITION_Y;
    }
    else
    {
        return -1;
...
{
    /*
     * Check to see if both (PB1) BTN8 and (PB2) BTN9 are pressed
     * simultaneously.
     */
    if (((ps_gpio_get_mio_state(pDemo, 50) == 1) &&
        (ps_gpio_get_mio_state(pDemo, 51) == 1)) ||
...
int zed_ali3_controller_demo_control(zed_ali3_controller_demo_t *pDemo)
{
    ...
    graphic.location_x = 21 + X_OFFSET;
    graphic.location_y = 20 + Y_OFFSET;
    ...
    zed_ali3_controller_demo_button(pDemo, 4, pDemo->button4_state);
    ...
    zed_ali3_controller_demo_switch(pDemo, 4, pDemo->switch4_state);
    zed_ali3_controller_demo_switch(pDemo, 5, pDemo->switch5_state);
    zed_ali3_controller_demo_switch(pDemo, 6, pDemo->switch6_state);
    zed_ali3_controller_demo_switch(pDemo, 7, pDemo->switch7_state);

    /*
     * Draw the OLED control.
     */
    graphic.location_x = 48 + X_OFFSET;
    graphic.location_y = 340 + Y_OFFSET;
    graphic.size_x = avnet_control_oled_width;
    graphic.size_y = avnet_control_oled_height;
    graphic.default_image_data = avnet_control_oled;
    draw_image_data(pDemo, &graphic);

    /*
	 * Draw the accelerometer panel and indicator.
	 */
    zed_ali3_controller_demo_accel_background(pDemo);
    zed_ali3_controller_demo_accel_indication(pDemo);
	// Wait for DMA to synchronize.
	Xil_DCacheFlush();
...
int zed_ali3_controller_demo_init(zed_ali3_controller_demo_t *pDemo)
{
	...
	// Blank the screen so that the configuration logo can be shown.
	draw_blank_screen(pDemo, COLOR_BLACK);

	// Wait for DMA to synchronize.
	Xil_DCacheFlush();

	// Draw the configuration splash image to the display.
    graphic.location_x = (pDemo->ali3_width / 2) - (avnet_control_system_config_width / 2);  // Centered on graphic
    graphic.location_y = (pDemo->ali3_height / 2) - (avnet_control_system_config_height / 2);  // Centered on graphic
    ...
    pDemo->switch4_state = 0;
    pDemo->switch5_state = 0;
    pDemo->switch6_state = 0;
    pDemo->switch7_state = 0;
    ...
    accelerometer_init(pDemo);

    haptic_driver_init(pDemo);
...
int zed_ali3_controller_demo_accel_background(zed_ali3_controller_demo_t *pDemo)
{
	...
	graphic.location_x = ACCEL_BACKGROUND_POSITION_X;
	graphic.location_y = ACCEL_BACKGROUND_POSITION_Y;

	graphic.default_image_data = avnet_control_accelerometer_led_plate;
	graphic.alternate_image_data = avnet_control_accelerometer_led_plate;

	// Set the width of the background panel graphic.
	graphic.size_x = avnet_control_accelerometer_led_plate_width;
	graphic.size_y = avnet_control_accelerometer_led_plate_height;
...
int zed_ali3_controller_demo_accel_indication(zed_ali3_controller_demo_t *pDemo)
{
    ...
    // All the LED images have the same width regardless of color or state 
	// of being ON or OFF.
    graphic.size_x = avnet_control_led_green_on_width;
    graphic.size_y = avnet_control_led_green_on_height;

	// Update the user interface to match the LED state change based upon 
	// latest data obtained from the accelerometer. 
    if (pDemo->accel_x > ACCEL_LOW_THRESHOLD && pDemo->accel_x < 128)
    {
        graphic.location_x = ACCEL_POSX_LOW_POSITION_X;
        graphic.location_y = ACCEL_POSX_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_on;
        graphic.alternate_image_data = avnet_control_led_green_off;
        graphic.size_x = avnet_control_led_green_on_width;
        graphic.size_y = avnet_control_led_green_on_height;
    }
    else
    {
        graphic.location_x = ACCEL_POSX_LOW_POSITION_X;
        graphic.location_y = ACCEL_POSX_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_off;
        graphic.alternate_image_data = avnet_control_led_green_on;
        graphic.size_x = avnet_control_led_green_off_width;
        graphic.size_y = avnet_control_led_green_off_height;
    }
    ...
    if (pDemo->accel_x > ACCEL_MED_THRESHOLD && pDemo->accel_x < 128)
    {
        graphic.location_x = ACCEL_POSX_MED_POSITION_X;
        graphic.location_y = ACCEL_POSX_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_on;
        graphic.alternate_image_data = avnet_control_led_yellow_off;
        graphic.size_x = avnet_control_led_yellow_on_width;
        graphic.size_y = avnet_control_led_yellow_on_height;
    }
    else
    {
        graphic.location_x = ACCEL_POSX_MED_POSITION_X;
        graphic.location_y = ACCEL_POSX_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_off;
        graphic.alternate_image_data = avnet_control_led_yellow_on;
        graphic.size_x = avnet_control_led_yellow_off_width;
        graphic.size_y = avnet_control_led_yellow_off_height;
    }
    ...
    if (pDemo->accel_x > ACCEL_HI_THRESHOLD && pDemo->accel_x < 128)
    {
        graphic.location_x = ACCEL_POSX_HIGH_POSITION_X;
        graphic.location_y = ACCEL_POSX_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_on;
        graphic.alternate_image_data = avnet_control_led_red_off;
        graphic.size_x = avnet_control_led_red_on_width;
        graphic.size_y = avnet_control_led_red_on_height;
    }
    else
    {
        graphic.location_x = ACCEL_POSX_HIGH_POSITION_X;
        graphic.location_y = ACCEL_POSX_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_off;
        graphic.alternate_image_data = avnet_control_led_red_on;
        graphic.size_x = avnet_control_led_red_off_width;
        graphic.size_y = avnet_control_led_red_off_height;
    }
    ...
    // Update the user interface to match the LED state change.
    if ((256 - pDemo->accel_x) > ACCEL_LOW_THRESHOLD && pDemo->accel_x > 128)
    {
        graphic.location_x = ACCEL_NEGX_LOW_POSITION_X;
        graphic.location_y = ACCEL_NEGX_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_on;
        graphic.alternate_image_data = avnet_control_led_green_off;
        graphic.size_x = avnet_control_led_green_on_width;
        graphic.size_y = avnet_control_led_green_on_height;
    ...
    if ((256 - pDemo->accel_x) > ACCEL_MED_THRESHOLD && pDemo->accel_x > 128)
    {
        graphic.location_x = ACCEL_NEGX_MED_POSITION_X;
        graphic.location_y = ACCEL_NEGX_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_on;
        graphic.alternate_image_data = avnet_control_led_yellow_off;
        graphic.size_x = avnet_control_led_yellow_on_width;
        graphic.size_y = avnet_control_led_yellow_on_height;
    ...
    if ((256 - pDemo->accel_x) > ACCEL_HI_THRESHOLD && pDemo->accel_x > 128)
    {
        graphic.location_x = ACCEL_NEGX_HIGH_POSITION_X;
        graphic.location_y = ACCEL_NEGX_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_on;
        graphic.alternate_image_data = avnet_control_led_red_off;
        graphic.size_x = avnet_control_led_red_on_width;
        graphic.size_y = avnet_control_led_red_on_height;
    ...
    // Update the user interface to match the LED state change.
    if (pDemo->accel_y > ACCEL_LOW_THRESHOLD && pDemo->accel_y < 128)
    {
        graphic.location_x = ACCEL_POSY_LOW_POSITION_X;
        graphic.location_y = ACCEL_POSY_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_on;
        graphic.alternate_image_data = avnet_control_led_green_off;
        graphic.size_x = avnet_control_led_green_on_width;
        graphic.size_y = avnet_control_led_green_on_height;
    ...
    if (pDemo->accel_y > ACCEL_MED_THRESHOLD && pDemo->accel_y < 128)
    {
        graphic.location_x = ACCEL_POSY_MED_POSITION_X;
        graphic.location_y = ACCEL_POSY_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_on;
        graphic.alternate_image_data = avnet_control_led_yellow_off;
        graphic.size_x = avnet_control_led_yellow_on_width;
        graphic.size_y = avnet_control_led_yellow_on_height;
    ...
    if (pDemo->accel_y > ACCEL_HI_THRESHOLD && pDemo->accel_y < 128)
    {
        graphic.location_x = ACCEL_POSY_HIGH_POSITION_X;
        graphic.location_y = ACCEL_POSY_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_on;
        graphic.alternate_image_data = avnet_control_led_red_off;
        graphic.size_x = avnet_control_led_red_on_width;
        graphic.size_y = avnet_control_led_red_on_height;
    ...
    // Update the user interface to match the LED state change.
    if ((256 - pDemo->accel_y) > ACCEL_LOW_THRESHOLD && pDemo->accel_y > 128)
    {
        graphic.location_x = ACCEL_NEGY_LOW_POSITION_X;
        graphic.location_y = ACCEL_NEGY_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_on;
        graphic.alternate_image_data = avnet_control_led_green_off;
        graphic.size_x = avnet_control_led_green_on_width;
        graphic.size_y = avnet_control_led_green_on_height;
    ...
    if ((256 - pDemo->accel_y) > ACCEL_MED_THRESHOLD && pDemo->accel_y > 128)
    {
        graphic.location_x = ACCEL_NEGY_MED_POSITION_X;
        graphic.location_y = ACCEL_NEGY_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_on;
        graphic.alternate_image_data = avnet_control_led_yellow_off;
        graphic.size_x = avnet_control_led_yellow_on_width;
        graphic.size_y = avnet_control_led_yellow_on_height;
    ...
    if ((256 - pDemo->accel_y) > ACCEL_HI_THRESHOLD && pDemo->accel_y > 128)
    {
        graphic.location_x = ACCEL_NEGY_HIGH_POSITION_X;
        graphic.location_y = ACCEL_NEGY_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_on;
        graphic.alternate_image_data = avnet_control_led_red_off;
        graphic.size_x = avnet_control_led_red_on_width;
        graphic.size_y = avnet_control_led_red_on_height;
    ...
    // Update the user interface to match the LED state change.
    if (pDemo->accel_z > ACCEL_LOW_THRESHOLD && pDemo->accel_z < 128)
    {
        graphic.location_x = ACCEL_POSZ_LOW_POSITION_X;
        graphic.location_y = ACCEL_POSZ_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_on;
        graphic.alternate_image_data = avnet_control_led_green_off;
        graphic.size_x = avnet_control_led_green_on_width;
        graphic.size_y = avnet_control_led_green_on_height;
    ...
    if (pDemo->accel_z > ACCEL_MED_THRESHOLD && pDemo->accel_z < 128)
    {
        graphic.location_x = ACCEL_POSZ_MED_POSITION_X;
        graphic.location_y = ACCEL_POSZ_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_on;
        graphic.alternate_image_data = avnet_control_led_yellow_off;
        graphic.size_x = avnet_control_led_yellow_on_width;
        graphic.size_y = avnet_control_led_yellow_on_height;
    ...
    if (pDemo->accel_z > ACCEL_HI_THRESHOLD && pDemo->accel_z < 128)
    {
        graphic.location_x = ACCEL_POSZ_HIGH_POSITION_X;
        graphic.location_y = ACCEL_POSZ_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_on;
        graphic.alternate_image_data = avnet_control_led_red_off;
        graphic.size_x = avnet_control_led_red_on_width;
        graphic.size_y = avnet_control_led_red_on_height;
    ...
    // Update the user interface to match the LED state change.
    if ((256 - pDemo->accel_z) > ACCEL_LOW_THRESHOLD && pDemo->accel_z > 128)
    {
        graphic.location_x = ACCEL_NEGZ_LOW_POSITION_X;
        graphic.location_y = ACCEL_NEGZ_LOW_POSITION_Y;
        graphic.default_image_data = avnet_control_led_green_on;
        graphic.alternate_image_data = avnet_control_led_green_off;
        graphic.size_x = avnet_control_led_green_on_width;
        graphic.size_y = avnet_control_led_green_on_height;
    ...
    if ((256 - pDemo->accel_z) > ACCEL_MED_THRESHOLD && pDemo->accel_z > 128)
    {
        graphic.location_x = ACCEL_NEGZ_MED_POSITION_X;
        graphic.location_y = ACCEL_NEGZ_MED_POSITION_Y;
        graphic.default_image_data = avnet_control_led_yellow_on;
        graphic.alternate_image_data = avnet_control_led_yellow_off;
        graphic.size_x = avnet_control_led_yellow_on_width;
        graphic.size_y = avnet_control_led_yellow_on_height;
    ...
    if ((256 - pDemo->accel_z) > ACCEL_HI_THRESHOLD && pDemo->accel_z > 128)
    {
        graphic.location_x = ACCEL_NEGZ_HIGH_POSITION_X;
        graphic.location_y = ACCEL_NEGZ_HIGH_POSITION_Y;
        graphic.default_image_data = avnet_control_led_red_on;
        graphic.alternate_image_data = avnet_control_led_red_off;
        graphic.size_x = avnet_control_led_red_on_width;
        graphic.size_y = avnet_control_led_red_on_height;
...
int zed_ali3_controller_demo_logo(zed_ali3_controller_demo_t *pDemo)
{
    ...
    // Blank the screen so that the logo can be drawn.
	draw_blank_screen(pDemo, COLOR_BLACK);
    ...
    start_row = 0;
    end_row = start_row + 800;

    start_col = 0;
    end_col = start_col + 1280;

    // Write the logo data to the display.
    pStorageMem = (uint32_t *)pDemo->uBaseAddr_MEM_FrameBuffer;
        ...
        for (row = 0; row < pDemo->ali3_height; row++)
        {
            for (col = 0; col < pDemo->ali3_width; col++)
            {
            	...
            	if ((((col > start_col) && (col < end_col)))&&(((row > start_row) && (row < end_row)))){
            		pixel = avnet_logo_image_data[(((row - start_row) * pDemo->ali3_width) + (col - start_col))];
            	}
            	else{
            		pixel = 0x00000000;
            	}
...
int zed_ali3_controller_demo_touch_calibrate(zed_ali3_controller_demo_t *pDemo)
{
	...
	// Blank the screen, so that the first calibration target can be drawn.
	draw_blank_screen(pDemo, COLOR_BLACK);
    ...
    ampire10_flush_touch_events(pDemo);
    ...
    while (result != 1)
    {
		...
		result = ampire10_process_touch_event(pDemo);
    ...
    // Blank the screen, so that the next calibration target can be drawn.
    draw_blank_screen(pDemo, COLOR_BLACK);
    ...
    // Blank the screen, so that the calibration targets are cleared away.
    draw_blank_screen(pDemo, COLOR_BLACK);
     ...
     */
    result = ampire10_get_calibration_matrix(pDemo, &touch_calibration_data, &touch_calibration_matrix);
...
int zed_ali3_controller_demo_touch_process(zed_ali3_controller_demo_t *pDemo)
{
    ...
    if (result == 1)
    {
        ...
        /*
         * Translate the touch location by using the latest touch calibration
         * parameters.
         */
        ampire10_translate_location(pDemo, &touch_location_raw, &touch_location_cal, &touch_calibration_matrix);
        ...
        /* Since buttons are used to control functionality in control mode,
         * the desire is to only detect button presses on the transition where
         * the finger touches down over the control and to avoid registering
         * new touches while the finger is down but may be reporting slightly
         * different co-ordintates.  Look only for the pen down transition in
         * this mode.
         */
        else if ((pDemo->mode == control) &&
        		 (pDemo->touch_pen_down_transition == 1))
        {
            ...
            {
            	haptic_trigger_effect(pDemo,STRONG_CLICK_100);
                ...
                (pDemo->touch_posy_cal >= LED1_POSITION_Y) && (pDemo->touch_posy_cal <= (LED1_POSITION_Y + avnet_control_led_off_height)))
            {
            	...
            	haptic_trigger_effect(pDemo,STRONG_CLICK_30);
                if (pDemo->bVerbose)
                {
                ...
                (pDemo->touch_posy_cal >= LED2_POSITION_Y) && (pDemo->touch_posy_cal <= (LED2_POSITION_Y + avnet_control_led_off_height)))
            {
            	...
            	haptic_trigger_effect(pDemo,SHARP_CLICK_100);
                if (pDemo->bVerbose)
                {
                ...
                (pDemo->touch_posy_cal >= LED3_POSITION_Y) && (pDemo->touch_posy_cal <= (LED3_POSITION_Y + avnet_control_led_off_height)))
            {
            	...
            	haptic_trigger_effect(pDemo,SHORT_DOUBLE_CLICK_STRONG_1_100);
                if (pDemo->bVerbose)
                {
                ...
                (pDemo->touch_posy_cal >= LED4_POSITION_Y) && (pDemo->touch_posy_cal <= (LED4_POSITION_Y + avnet_control_led_off_height)))
            {
            	...
            	haptic_trigger_effect(pDemo,TRANSITION_HUM_1_100);
                if (pDemo->bVerbose)
                {
                ...
                (pDemo->touch_posy_cal >= LED5_POSITION_Y) && (pDemo->touch_posy_cal <= (LED5_POSITION_Y + avnet_control_led_off_height)))
            {
            	...
            	haptic_trigger_effect(pDemo,TRANSITION_RAMP_DOWN_LONG_SMOOTH_1_100_TO_0);
                if (pDemo->bVerbose)
                {
                ...
                (pDemo->touch_posy_cal >= LED6_POSITION_Y) && (pDemo->touch_posy_cal <= (LED6_POSITION_Y + avnet_control_led_off_height)))
            {
                ...
                haptic_trigger_effect(pDemo,TRANSITION_RAMP_UP_LONG_SMOOTH_2_0_TO_100);
                if (pDemo->bVerbose)
                {
                ...
                (pDemo->touch_posy_cal >= LED7_POSITION_Y) && (pDemo->touch_posy_cal <= (LED7_POSITION_Y + avnet_control_led_off_height)))
            {
                ...
                haptic_trigger_effect(pDemo,TRANSITION_RAMP_DOWN_SHORT_SHARP_1_50_TO_0);
                if (pDemo->bVerbose)
                {
...
int zed_ali3_controller_demo_touch_status(zed_ali3_controller_demo_t *pDemo)
{
	...
	result = ampire10_read_touch_data(pDemo);
...
int zed_ali3_controller_demo_switch(zed_ali3_controller_demo_t *pDemo, int switch_number, int switch_state)
{
    ...
    {
        pDemo->switch4_state = switch_state;

        graphic.location_x = SWITCH4_POSITION_X;
        graphic.location_y = SWITCH4_POSITION_Y;
    }
    else if (switch_number == 5)
    {
        pDemo->switch5_state = switch_state;

        graphic.location_x = SWITCH5_POSITION_X;
        graphic.location_y = SWITCH5_POSITION_Y;
    }
    else if (switch_number == 6)
    {
        pDemo->switch6_state = switch_state;

        graphic.location_x = SWITCH6_POSITION_X;
        graphic.location_y = SWITCH6_POSITION_Y;
    }
    else if (switch_number == 7)
    {
        pDemo->switch7_state = switch_state;

        graphic.location_x = SWITCH7_POSITION_X;
        graphic.location_y = SWITCH7_POSITION_Y;
    }
    else
    {
        return -1;
