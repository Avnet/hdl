...
void avnet_console_process(avnet_console_t *pConsole)
{
    ...
    else if ( !strcmp(cargv[0],"pcap") )
    {
        avnet_console_pcap_command(pConsole, cargc, cargv);
    }
...
void avnet_console_banner_command(avnet_console_t *pConsole)
{
    pConsole->io_hprintf(pConsole->io_handle, "\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "------------------------------------------------------------------------------\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                       ~I$OOZOOOZO8I            ZOOZZZOO8OOOOOZ7=           |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                     .ZZI??+==~=+=:              ~+=:+==~=+=:ZOZ87          |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                    +O$:   :~==~                             ~OOOZ:         |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                   $8$ +$$$7I??7ZOOZZOZZZZZZZOZO8OZOZOOOZ7    ZOO8          |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                  ZOO  8$?I?III?I$888OOO88888OZZ88OOOOOZ8I   ?OO8I          |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                 +OO+  87?IIIIII?I7$ZO88888O$I?+?$Z8OOOOO+   $OOO?          |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                 7I   $OZ7III$O8888OZ$I????????I???Z8OO8$   ~O88Z           |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                     7O8888888O88OOOOODD$?IIIII?I??Z8OOO=    .:,            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                    ~888OO8OOO8O888888O$I?III?III7Z88O8Z                    |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                    Z8888888888888DDO77$ZZZ$7?7$O8888O8?                    |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                   Z8O88O$$7$ZOZ$77$OD888888888888888OZ                     |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                   888DZ$77777777Z8D888888888888888888$                     |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                  8888Z77777777ODD8O8D8888888888888888                      |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|           ==    88888$777777777Z8DDD888888DD88888888$                      |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|          Z88   $88888Z$$$$$ZZZZZ$$$ZO8888OZ$7777$O8D:                      |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         78D+  ?8D8888DOOZO8DN8DDD88Z$77$7777777II7ZD                       |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|        $8D?   DD8888D888DDD88D8888DDD8Z77$777$$7$778    788+.              |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|        D8O   ZDDDD88D88888888D88DD888D8Z7$$7$77777$8   78D8O               |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|       D8D    $O8888D8888DDDDD8DDDD88DDDO$77777$$77ZN   D88DI               |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|      D8DNZ:            :=?I$ZO8DDDDDDDDDDO$$Z$77ZNM8  $DD8D                |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|       D8DD8OZ77?+==~,          ,::~=+??$8ND8888DDDO7  8D8NZ                |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|        ZDDD8DDDNDNDD8~                   I8DDDDD8Z+  7DDDD?                |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|            =I$ODDDDD7                I=             8NDD8$                 |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                  =I+               .NDDDNDZ7      +$DNDND                  |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                                    ONDNNNNDDDDDDDDNNDNO+                   |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                                         I$O8DDNNNDDD?                      |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                                               ~===:                        |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                                       ......                ..             |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                                      $OOZO8OO~             7OO~ TM         |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|                  ??                       Z8:               7O:            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         ......        ....    .....     .ZOI    ....     ...$O:            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         II7??$7  II ,II7III, +7III$+   :O8.    OD778Z~ $8ZZ8OO:            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         77:  ~$+ 77 ?I:   ?? 7I   I7,  88+    I8----8$ 88   88:            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         77,  ~$I 77 77,      7I   I7: 8D=  +O $D       8O   O8:            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         7$$II$$, $7 ,Z$?I$7I ?$I?I$I DD8ZOO88+ DN7I8DO 8DO7ODD:            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         Z$ :=    ~~    +=:     ~+~   =++++++?:  ,$I+     ?I+ 8.            |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         $$                                                                 |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         $$7                                                                |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "|         :::                                                                |\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "------------------------------------------------------------------------------\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "--                      Zed Display Kit Demonstration                       --\n\r");
    pConsole->io_hprintf(pConsole->io_handle, "------------------------------------------------------------------------------\n\r");
...
void avnet_console_help(avnet_console_t *pConsole)
{
    ...
    pConsole->io_hprintf(pConsole->io_handle, "\tiic        IIC accesses on Zed Display Kit\n\r");
    ...
    pConsole->io_hprintf(pConsole->io_handle, "\tpcap       Display status of last PCAP touch event\n\r");
...
void avnet_console_pcap_command(avnet_console_t *pConsole, int cargc, char ** cargv)
{
    ...
    {
        if (strcmp( cargv[1], "baseline") == 0)
        {
            if (demo.controller_type == clicktouch)
            {
                /* Perform the baseline calibration/adjustement operation on the touch controller. */
                num_bytes = tmg120_baseline_clicktouch(&demo);

                xil_printf( "PCAP Touch: Reset baseline command (%d bytes long) sent to Clicktouch sensor.\n\r",  num_bytes );
            }
            else
            {
                xil_printf( "PCAP Touch: No baseline reset command available for detected type of touch device. \n\r");
            }
        }
        else if (strcmp( cargv[1], "idac") == 0)
        {
            if (demo.controller_type == clicktouch)
            {
                /* Perform the IDAC calibration on the touch controller. */
                num_bytes = tmg120_recalibrate_idac_clicktouch(&demo);

                xil_printf( "PCAP Touch: Recalibrate IDAC command (%d bytes long) sent to Clicktouch sensor.\n\r",  num_bytes );
            }
            else
            {
                xil_printf( "PCAP Touch: No recalibrate IDAC command available for detected type of touch device. \n\r");
            }
        }
        else if (strcmp(cargv[1], "last") == 0)
        {
            xil_printf( "PCAP Touch:IRQs=%4d,VALID=%4d,EVENTs=%4d,DUPS=%4d,OF=%4d,X=%4d,Y=%4d\n\r",
                demo.touch_irqs, demo.touch_valid, demo.touch_events, demo.touch_duplicates, demo.touch_overflows, demo.touch_posx_raw, demo.touch_posy_raw );
        }
        else if (strcmp( cargv[1], "reset") == 0)
        {
            if (demo.controller_type == clicktouch)
            {
                /* Perform a reset request on the touch controller. */
                num_bytes = tmg120_reset_clicktouch(&demo);

                xil_printf( "PCAP Touch: Reset command (%d bytes long) sent to Clicktouch sensor.\n\r",  num_bytes );
            }
            else
            {
                xil_printf( "PCAP Touch: No Reset command available for detected type of touch device. \n\r");
            }
        }
        else
        {
    ...
    {
        pConsole->io_hprintf(pConsole->io_handle, "\tSyntax :\r\n");
        pConsole->io_hprintf(pConsole->io_handle, "\t\t%s baseline            => Reset baseline\n\r", cargv[0]);
        pConsole->io_hprintf(pConsole->io_handle, "\t\t%s idac                => Recalibrate IDAC values\n\r", cargv[0]);
        pConsole->io_hprintf(pConsole->io_handle, "\t\t%s last                => Show last raw data from touch sensor\n\r", cargv[0]);
        pConsole->io_hprintf(pConsole->io_handle, "\t\t%s reset               => Reset touch sensor\n\r", cargv[0]);
    }
}
