...
int main() {
	...
	pdemo = &demo;
	pdemo->ppzsdr_fmccc_iic = &(pdemo->pzsdr_fmccc_iic);
	pdemo->pcfa = &(pdemo->cfa);
	pdemo->pccm = &(pdemo->ccm);
	pdemo->paxivdma = &(pdemo->axivdma);
	pdemo->posd = &(pdemo->osd);

   status = fmc_iic_xps_init(pdemo->ppzsdr_fmccc_iic,"PZSDR FMCCC I2C Controller", XPAR_PZSDR_FMCCC_IIC_0_BASEADDR );
   if ( !status )
   {
	...
	// I2C MUX Reset
	tca9548_i2c_mux_select(pdemo->ppzsdr_fmccc_iic, EMBV_IIC_MUX_I2CIO);
	status = 0x08;
	pca9534_set_pins_direction(pdemo->ppzsdr_fmccc_iic, status);
	pca9534_set_pin_value(pdemo->ppzsdr_fmccc_iic, 7, 1);		// Disable I2C MUX Reset

	// HDMI Ouput Initialization
	//tca9548_i2c_mux_select(pdemo->ppzsdr_fmccc_iic, EMBV_IIC_MUX_I2CIO);
	//pca9534_set_pin_value(pdemo->ppzsdr_fmccc_iic, 4, 0);// Disable HDMI Output Power Down

	tca9548_i2c_mux_select(pdemo->ppzsdr_fmccc_iic, TCA9548_I2C2_SEL); // I2C_ADV7511
	adv7511_configure(pdemo->ppzsdr_fmccc_iic);

	// ISERDES Reset Assert
	Xil_Out32(XPAR_TCM_RECEIVER_0_S00_AXI_BASEADDR + 0x0000, 0x0001);

	// TCM Initialization
	tca9548_i2c_mux_select(pdemo->ppzsdr_fmccc_iic, TCA9548_I2C4_SEL); // I2C_CAMERA
	tcm5117pl_get_chip_id(pdemo->ppzsdr_fmccc_iic);
	tcm5117pl_init(pdemo->ppzsdr_fmccc_iic, TCM5117PL_1080P60);
